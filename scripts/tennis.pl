...
# HTTP::Tiny verifies the certificate but has to be told where the trust store
# is: it looks for $ENV{SSL_CERT_FILE}, then Mozilla::CA, and neither is
# guaranteed on a box that has IO::Socket::SSL from its package manager. Every
# distribution ships a bundle somewhere, so look there first and only fall back
# to whatever HTTP::Tiny can find by itself. Without this a request fails with
# "Couldn't find a CA bundle" on an otherwise perfectly capable system.
sub ca_bundle {
    return $ENV{SSL_CERT_FILE} if $ENV{SSL_CERT_FILE} && -r $ENV{SSL_CERT_FILE};
    for my $path (
        '/etc/ssl/certs/ca-certificates.crt',   # Debian, Ubuntu, Arch, Alpine
        '/etc/pki/tls/certs/ca-bundle.crt',     # Fedora, RHEL
        '/etc/ssl/ca-bundle.pem',               # openSUSE
        '/etc/ssl/cert.pem',                    # OpenBSD, FreeBSD, macOS brew
